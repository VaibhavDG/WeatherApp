                ...
                if let weather = viewModel.weather {
                    ...
                    ScrollView{
                        ...
                        VStack(spacing: 20) {
                            ...
                            Text("\(TemperatureConverter.convert(weather.temperature, to: settingsViewModel.temperatureUnit))\(settingsViewModel.temperatureUnit.symbol)")
                                .font(.system(size: 72, weight: .bold))
                                .foregroundColor(.white)
                            
                            HStack(spacing: 16) {
                                Text("H: \(TemperatureConverter.convert(weather.highTemperature, to: settingsViewModel.temperatureUnit))\(settingsViewModel.temperatureUnit.symbol)")
                                    .font(.title3)
                                    .foregroundColor(.white.opacity(0.9))
                                
                                Text("L: \(TemperatureConverter.convert(weather.lowTemperature, to: settingsViewModel.temperatureUnit))\(settingsViewModel.temperatureUnit.symbol)")
                                    .font(.title3)
                                ...
                                ForEach(weather.forecast.prefix(5)) { day in
                                    VStack(spacing: 4) {
                                        ...
                                        Image(systemName: day.icon)
                                            .font(.title3)
                                        VStack(spacing: 2) {
                                            Text("\(TemperatureConverter.convert(day.highTemperature, to: settingsViewModel.temperatureUnit))\(settingsViewModel.temperatureUnit.symbol)")
                                                .font(.caption)
                                                .foregroundColor(.white)
                                            Text("\(TemperatureConverter.convert(day.lowTemperature, to: settingsViewModel.temperatureUnit))\(settingsViewModel.temperatureUnit.symbol)")
                                                .font(.caption2)
                                                .foregroundColor(.white.opacity(0.7))
                                        }
                                    }
